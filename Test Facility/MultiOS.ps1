...
Echo ""
Echo ""
$port135 = (Porttest "$pcip" 135)
write-host "	135 (Windows) $port135" 
IF ($port135 -eq "True"){
GetWinMenu
break
}

$port88 = (Porttest "$pcip" 88)
write-host "	88 (Apple) $port88" 
IF ($port88 -eq "True"){
GetMacMenu
break
}

$port22 = (Porttest "$pcip" 22)
write-host "	22 (Linux/Mac) $port22" 
IF ($port22 -eq "True"){
GetLinuxMenu
}
...
function GetMacMenu {
Echo ""
Echo "No commands currently exist for Apple Based Devices." 
Echo "If you have any ideas, or scripts for apple reconnaisance, please feel free to edit the code, and insert it!"
pause
GetCompName
}
function GetLinuxMenu {
Echo ""
write-host "This device IS most Likely Unix based, However I cannot be 100% certain."
}
...
function CheckHost{
    ...
    if($ping.StatusCode -eq 0){$pcip=$ping.ProtocolAddress; getos} 
...
function GetInfosoftware{ 
    ...
    switch ($MenuSelectionsoftware){ 
        ...
        4 { #OPEN
			write-host "Have an idea for a command? Open the code, and add it!"
			Pause 
            GetMenusoftware 
          } 

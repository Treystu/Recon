#Calling Port Check Script
. C:\arcsight\scripts\Recon\Test-PortWin.ps1 | out-null
...
function CheckHost{
    ...
    if($ping.StatusCode -eq 0){$pcip=$ping.ProtocolAddress; PortCheck} 
    else{Pause "Host $compname down...Press any key to continue"; GetCompName} 
} 
##TESTING##
#Check port 135 for Windows compatibility

function NullCheck{
$result = $null
$result = .\Test-PortWin $pcip 135
IF($result -eq ""){
write-host "The Address Variable has not been passed."
GetCompName
}
ELSE{PortCheck
}
